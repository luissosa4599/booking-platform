...
function buildDayBuckets(slots: AvailabilitySlot[]): DayBucket[] {
  ...
  return Array.from({ length: DAYS_SHOWN }, (_, index) => {
    ...
    return {
      date,
      label: index === 0 ? "Hoy" : date.toLocaleDateString("es-MX", { weekday: "short" }),
...
export default function ResourceScreen() {
  ...
  const { id, name: passedName, location: passedLocation } = useLocalSearchParams<{
    id: string;
    name?: string;
    location?: string;
  }>();
  ...
  const [joinedWaitlistSlotIds, setJoinedWaitlistSlotIds] = useState<Set<string>>(new Set());
  ...
  const resourceType = resourceTypesQuery.data?.find((t) => t.id === resource?.resourceTypeId);
  ...
  const days = useMemo(() => buildDayBuckets(resource?.upcomingSlots ?? []), [resource]);
  ...
  const selectedSlot =
    rawSelectedSlot && rawSelectedSlot.capacityRemaining >= seatCount ? rawSelectedSlot : null;

  const stepperMax = selectedSlot ? selectedSlot.capacityRemaining : (resource?.capacity ?? 1);
  const showSlotSkeleton = useDelayedFlag(resourceQuery.isLoading, 150);

  const conflictSlot = createBooking.conflict
    ? (resource?.upcomingSlots.find((s) => s.id === createBooking.conflict!.availabilitySlotId) ??
      null)
    : null;
  ...
  const subtitleParts = [
    ...
    resource ? `hasta ${resource.capacity} ${capacityUnitLabel}` : passedLocation,
  ...
  const ctaLabel = selectedSlot ? `${actionVerb} ${formatTime(selectedSlot.startsAt)}` : "Elige un horario";
  ...
  function handleJoinWaitlist(slot: AvailabilitySlot) {
    ...
    joinWaitlist.mutate(
      ...
      { onSuccess: () => setJoinedWaitlistSlotIds((prev) => withId(prev, slot.id)) },
    );
  }
  ...
  function handleConfirm() {
    ...
    createBooking.mutate(
      { availabilitySlotId: bookedSlot.id, userId: demoUserId, seats: seatCount },
      {
        onSuccess: () => {
          setShowSuccess(true);
          setTimeout(() => {
            useToastStore
              .getState()
              .show(`${resource.name} · hoy ${formatTime(bookedSlot.startsAt)}`, "Ver");
            router.back();
          }, SUCCESS_PAUSE_MS);
        },
      },
        ...
        <View className="gap-6 px-4 pt-6">
          <View className="gap-[6px]">
            ...
            {subtitleParts.length > 0 ? (
              <Text className="text-body text-label-3">{subtitleParts.join(" · ")}</Text>
            ) : null}
          </View>

          {allowsMultipleSeats ? (
            <View className="flex-row items-center justify-between px-1">
              <Text className="text-[17px] text-label-1">{capitalize(capacityUnitLabel)}</Text>
              <Stepper value={seatCount} max={stepperMax} onChange={setSeatCount} />
            </View>
          ) : null}
            ...
            <View className="flex-row gap-2">
              {days.map((day, index) => {
                ...
                return (
                  ...
                  >
                    <Text className="text-[13px] text-label-4">{day.label}</Text>
                    <Text
                      className={cn(
                        "text-body-emph",
            ...
            <Group variant="canvas">
              {showSlotSkeleton ? (
                ...
                daySlots.map((slot) => {
                  ...
                  const isJoining =
                    joinWaitlist.isPending && joinWaitlist.variables?.availabilitySlotId === slot.id;
                    ...
                    if (allowsWaitlist) {
                      return (
                        <Row
                          ...
                          onPress={isJoined ? undefined : () => handleJoinWaitlist(slot)}
                        />
                      );
                    }
                    return (
                  ...
                  return (
                    <Row
                      ...
                      trailing="text"
                      trailingText={
                        slot.capacityRemaining === 1 ? "Último lugar" : `${slot.capacityRemaining} lugares`
                      }
                      trailingTone={slot.capacityRemaining === 1 ? "last" : "default"}
                      selected={slot.id === selectedSlotId && eligible}
                      disabled={!eligible}
                      onPress={eligible ? () => setSelectedSlotId(slot.id) : undefined}
                    />
                  );
                })
              ) : (
                <View className="px-4 py-6">
                  <Text className="text-body text-label-3">No hay horarios este día.</Text>
                </View>
              )}
            </Group>
          </View>
